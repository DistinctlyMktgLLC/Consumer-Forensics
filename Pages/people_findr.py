...
def app():
    ...
    marstat = st.sidebar.multiselect("Marital Status", data_sampled['status'].dropna().unique(), help="Demographic - Marital Status")
    educ = st.sidebar.multiselect("Education", data_sampled['education'].dropna().unique(), help="Demographic - Education")
    employ = st.sidebar.multiselect("Employment", data_sampled['employment'].dropna().unique(), help="Demographic - Employment")
    ...
    if marstat:
        filtered_data = filtered_data[filtered_data['status'].isin(marstat)]
    if educ:
        filtered_data = filtered_data[filtered_data['education'].isin(educ)]
    if employ:
        filtered_data = filtered_data[filtered_data['employment'].isin(employ)]
    if gender:
    ...
    tooltip = {
        "html": "<b>Sign:</b> {sign}<br/>"
                ...
                "<b>Race:</b> {race}<br/>"
                "<b>Marital Status:</b> {status}<br/>"
                "<b>Education:</b> {education}<br/>"
                "<b>Employment:</b> {employment}<br/>"
